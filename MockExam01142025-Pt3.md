...
**1. A Generative AI Engineer is tasked with creating a chatbot that provides customer service for an online retail store. Which prompt would elicit a specifically formatted response for customer inquiries about product returns?**\
...
**2. A business use case involves creating an AI assistant to summarize legal contracts. Which chain component would best extract relevant sections of the contracts before summarization?**\
A. Text Generator\
B. Text Splitter\
C. Summarizer\
D. Search Engine

**3. A company wants to design a pipeline for an AI application that generates meeting agendas from emails. What should the pipeline inputs and outputs look like?**\
...
**4. What is the first step in translating a business use case goal into an AI pipeline?**\
...
**5. Which tool would help gather knowledge for a multi-stage reasoning application?**\
A. Vector Search\
B. Data Lakehouse\
C. Retrieval-Augmented Generation\
D. Database Query

**6. A Generative AI Engineer is designing a customer service assistant that returns formatted email responses. What is the best way to ensure consistent formatting?**\
...
**7. An AI Engineer is creating a multi-stage pipeline for answering technical support questions. What should the pipeline’s second stage focus on?**\
...
**8. A Generative AI Engineer is working with a legal document split into sections. Which chunking strategy would best handle these documents for model constraints?**\
A. Random splitting\
B. Split by sentences\
C. Split by semantic paragraphs\
D. Split by word count

**9. To filter extraneous content in a document for a RAG application, what should the engineer prioritize?**\
A. Removing unrelated metadata\
B. Compressing large files\
C. Adding extra formatting\
D. Including all document content

**10. Which Python package is best for extracting tabular data from a PDF file?**\
A. NumPy\
B. Pandas\
C. PyPDF2\
D. Tabula

**11. After chunking text, how should the data be written into Delta Lake tables in Unity Catalog?**\
...
**12. A RAG application needs high-quality knowledge sources. Which documents should the engineer prioritize?**\
A. General web articles\
B. Peer-reviewed papers and internal knowledge bases\
C. Social media posts\
D. Random blogs

**13. Which step is critical for aligning prompt/response pairs with a specific model task?**\
...
**14. What is a common metric to evaluate retrieval performance in RAG applications?**\
A. Query embedding dimensions\
B. Retrieval precision\
C. Token count per document\
D. Query latency

**15. A document for a RAG application contains large headers and advertisements. What should the engineer do to improve input quality?**\
...
**16. Which tool should be used to extract structured data from a REST API for a RAG application?**\
A. SQL\
B. JSON Parser\
C. LangChain Toolkits\
D. Pandas

**17. A prompt asks for a detailed breakdown of a financial report but returns incomplete information. What should be changed?**\
A. Increase the context length\
B. Add a few-shot example to the prompt\
C. Use a smaller model\
D. Reduce input data complexity

**18. How can an engineer prevent an LLM from generating harmful or offensive content?**\
...
**19. When building a RAG application, what strategy best mitigates hallucinations?**\
A. Increase model size\
B. Use retrieval-based context augmentation\
C. Enable model chaining\
D. Reduce data pre-processing

**20. To create an LLM application that adjusts responses based on user input, what should be implemented?**\
A. Static templates\
B. Dynamic prompt augmentation\
C. Basic output filtering\
D. Simplified chaining

**21. Which strategy would help minimize hallucinations in LLM responses?**\
...
**22. What is the most effective way to augment a prompt with user-specific context?**\
...
**23. How can developers add guardrails to an LLM application?**\
...
**24. Which model card metric is most relevant when selecting a model for summarization tasks?**\
A. BLEU score\
B. Inference time\
C. Model parameter count\
D. Dataset size

**25. An LLM produces inconsistent results for similar queries. What should an engineer test?**\
...
**26. What is the key element of coding a chain using a pyfunc model?**\
...
**29. What is the correct sequence for deploying an endpoint in Databricks for a RAG application?**\
...
**30. How is a vector search index queried in a typical RAG system?**\
...
**31. What’s a critical resource needed for serving features in a RAG application?**\
A. High-precision embeddings\
B. Cloud storage scalability\
C. GPU-based model hosting\
D. Custom data pipelines

**32. How can you restrict access to an LLM API endpoint?**\
...
**33. What should be prioritized when deploying an AI application for high-availability use cases?**\
A. Model size\
B. Load balancing\
...
**34. Which masking technique would meet data privacy objectives for an LLM?**\
A. Hashing sensitive fields\
B. Removing all data\
C. Providing plaintext data\
D. Encrypting the entire dataset

**35. How can engineers prevent malicious user inputs in an LLM application?**\
A. Ignore edge cases\
B. Use input validation and filtering\
C. Block all inputs\
D. Deploy a simple system prompt

**36. A company is concerned about licensing issues when using external data sources for an LLM. What should be their first step?**\
...
**37. What governance strategy can ensure ethical LLM deployment?**\
...
**44. Which monitoring tool would help assess user engagement with an AI application?**\
A. Latency monitors\
B. Session analytics tools\
C. Memory profiling tools\
D. Query embedding evaluations

**45. What’s the most effective way to track hallucinations in an LLM response?**\
